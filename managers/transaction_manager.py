...
producer = Producer('transactions')
...
class TransactionManager:
    ...
    def deposit(account, amount):
        ...
        account.update_account()
        AccountManager.update_account_balance(account)
        return TransactionManager.add_transaction(amount, TransactionTypes.deposit, account)


    @staticmethod
    def add_transaction(amount, transaction_type, account):
        transaction = Transaction(amount, transaction_type, account)
        producer.send(transaction.get_dictionary())
        return transaction
